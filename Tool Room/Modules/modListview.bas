...
Public Sub LvFullRow(ByRef ListView As ListView)
...
Public Sub LvGrid(ByRef ListView As ListView)
...
Public Function LvReturnSelected(ListView As ListView) As ListItem

    Set LvReturnSelected = Nothing
    ...
    For lvIndex = 1 To lvCount
        If ListView.ListItems(lvIndex).Selected = True Then
            Set LvReturnSelected = ListView.ListItems(lvIndex)
            Exit Function
        End If
    Next lvIndex
